...
(function( core ){
    ...
    */
    core.localStorage.setStorage = function (name, data, expiry, useCookie, domain) {
        ...
        var setCookie = function(){
            ...
            var domain_value = domain ? " domain=" + ( domain || window.location.host ) + ";" : "";
            document.cookie = name + "=" + c_value + domain_value + " path=/";
        };
        if (useCookie) {
            setCookie();
        }
...
(function( ui, core ){
	...
	var buildNavs = function( _self ){
		...
		if ( _self.config.navItemsWrap.children ) {
			_self.numMenuItems = _self.config.navItemsWrap.children.length;
		}

		_self.aboveTriggerThreshold = _self.numMenuItems >= _self.config.minTrigger;
			...
			_self.config.navs.forEach(function(nav, i) {
				if(!nav.isMoreNav) {
					...
					if(widthRemaining !== -1) {
						...
						if ( _self.aboveTriggerThreshold ) {
